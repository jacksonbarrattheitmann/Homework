...
The model residuals for both of these models do not seem to conform to normality. The residuals are fairly kurtotic, meaning the models most likely deviate from normal assumptions.  
...
3. *Acer Rubrum* and *Abies fraseri* cover are both explained best by the elevation variable. Acer occurs across the elevation gradient while *Abies* only occurs at higher elevations, and is probably why the model fits considerably better for *Abies* than *Acer*, because their distributions as shown in the plots above differ greatly. *Acer* actually has a negative slope, meaning that as elevation goes up, *Acer rubrum* cover goes down. *Abies fraseri* cover has a positive slope as elevation increases. *Abies* had considerably more 0s in the dataset, which could be why model fit was better overall because there was certainly an elevation threshold for that species' distribution, whereas *Acer* occurs at all elevations < 1500 m. 
...
both the Acer rubrum model andAbies fraseri model. The stepAIC output for the Abies fraseri model includes both tci and streamdist, which only reduce the AIC fit by 1 and 3 units respectively, which make my confident that they could be excluded from the original lm model I made, and even though statistically significant, I am not convinced that they are necessary. 
